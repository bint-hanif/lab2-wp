...
function start() { 
    ...
    //create bees
    makeBees();
    //when the input field of bear speed is changed, function setSpeed is invoked
    document.getElementById("speedBear").addEventListener("change", setSpeed);        
    updateBees();
    //when a key pressed, setLastStingTime() function is called 
    document.addEventListener("keydown", setLastStingTime);
    //when the button 'add bee' is clicked, the function addBee() is called 
    }

//instantiates lastStingTime 
function setLastStingTime(){
...
//sets dBear to the new speed of the bear after the input is changed
function setSpeed() {
...
function updateBees() { // update loop for game
    ...
    moveBees();
    //use period time inputted by user
    let period = document.getElementById("periodTimer").value;
    //check if the number of stings reaches 1000
...
function addBee(){
    //adds one to the number of bees, create a new bee, display it and add it to the array of bees
    var bee = new Bee(); //create object and its IMG element
    bee.display(); //display the bee
    bees.push(bee); //add the bee object to the bees array
}
